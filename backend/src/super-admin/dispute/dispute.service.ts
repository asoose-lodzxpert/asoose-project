...
export class DisputesService {
  ...
  private async validateDisputeEligibility(dto: CreateDisputeDto, userId: string) {
    if (dto.orderId) {
      ...
      if (order.userId !== userId) throw new ForbiddenException('You can only dispute your own orders');
      ...
      if (ride.customerId !== userId) throw new ForbiddenException('You can only dispute your own rides');
      ...
      const delivery = await this.prisma.delivery.findUnique({ where: { id: dto.deliveryId } });
      if (!delivery) throw new NotFoundException('Delivery not found');
      if (delivery.customerId !== userId) throw new ForbiddenException('You can only dispute your own deliveries');
    }
  }
  ...
  async findAll(params: {
    ...
    const isAdmin = ([
      UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.ADMIN_SUPPORT, UserRole.ADMIN_MANAGER,
  ...
  async findOne(id: string, userId: string, role: string | UserRole) {
    const isAdmin = ([UserRole.SUPER_ADMIN, UserRole.ADMIN_MANAGER, UserRole.ADMIN_SUPPORT] as UserRole[])
    ...
    const siblings = dispute.order?.orderGroup?.orders.filter(o => o.id !== dispute.orderId) || [];
  ...
  async resolve(id: string, dto: ResolveDisputeDto, adminId: string) {
    ...
    if ((dispute as any).refundStatus === 'PROCESSING') {
      throw new ConflictException('A refund is currently processing for this dispute. Please check logs/gateway.');
    }
   ...
   */
  private async executeSafeRefundFlow(dispute: any, dto: ResolveDisputeDto, adminId: string) {
    ...
    const maxRefund = dispute.order?.total || 
                      dispute.ride?.totalFare || 
                      dispute.delivery?.deliveryFee || 
                      effectivePayment?.amount || 0;

    let refundAmount = 0;
    if (dto.action === ResolutionAction.REFUND_PARTIAL) {
      if (!dto.refundAmount || dto.refundAmount <= 0) throw new BadRequestException('Refund amount must be greater than 0');
      if (dto.refundAmount > maxRefund) throw new BadRequestException(`Refund amount cannot exceed ₦${maxRefund}`);
      refundAmount = dto.refundAmount;
    } else {
      refundAmount = maxRefund;
    }

    if (refundAmount <= 0) throw new BadRequestException('Refund amount invalid');
    if (!effectivePayment?.reference) throw new BadRequestException('No valid payment reference found for refund.');
...
let gatewayResponse: { refundReference: string } | null = null;
    ...
    try {
      this.logger.log(`[Dispute ${dispute.id}] Initiating Gateway Refund. Key: ${idempotencyKey}`);

      const isTestTransaction = effectivePayment.reference.startsWith('PAY-REF-') || effectivePayment.reference.startsWith('REF-');
      ...
      if (!isTestTransaction) {
        ...
        this.logger.warn(`[Dispute ${dispute.id}] Skipping gateway call for Test Transaction`);
        gatewayResponse = { refundReference: 'TEST-REFUND-' + randomUUID() };
      }

    } catch (error) {
      this.logger.error(`[Dispute ${dispute.id}] Refund Failed: ${error.message}`, error.stack);
      ...
      throw new BadRequestException(`Refund execution failed: ${error.message}`);
    }
    ...
    try {
      const result = await this.prisma.$transaction(async (tx) => {
        // A. Ledger Entries
        await this.recordRefundLedger(tx, dispute, refundAmount, dto, adminId, effectivePayment);

        // B. Update Payment Status
        if (effectivePayment) {
          const isFullGroupRefund = !isGroupTransaction && dto.action === ResolutionAction.REFUND_FULL;
          await tx.payment.update({
             where: { id: effectivePayment.id },
             data: { status: isFullGroupRefund ? 'REFUNDED' : 'PARTIALLY_REFUNDED' } 
          });
        }
      ...
      if (dispute.openedByUserId) {
        await this.notificationsService.sendToUser(dispute.openedByUserId, {
      ...
      this.logger.error(`[Dispute ${dispute.id}] CRITICAL: Gateway Refunded but DB Update Failed!`, dbError.stack);
      throw new InternalServerErrorException('Refund successful, but system record update failed. Please contact support do NOT retry.');
    }
  }
   ...
   */
  private async executeStandardResolution(dispute: any, dto: ResolveDisputeDto, adminId: string) {
    const result = await this.prisma.$transaction(async (tx) => {
      const updated = await tx.dispute.update({
          where: { id: dispute.id },
          data: {
   ...
   */
  private async recordRefundLedger(tx: any, dispute: any, amount: number, dto: ResolveDisputeDto, adminId: string, effectivePayment: any) {
    // 1. Platform Record
    await tx.transaction.create({
        data: {
            ...
            ...(dispute.order?.orderGroupId && { orderGroupId: dispute.order.orderGroupId }),
    ...
    if (dto.refundSource === RefundSource.VENDOR_WALLET && dispute.order?.storeId) {
        ...
        if (currentBalance < amount) {
          this.logger.warn(`Store ${dispute.order.storeId} balance will go negative.`);
        }
  ...
  async addMessage(id: string, dto: AddMessageDto, userId: string, role: string) {
    ...
    if (!dispute) throw new NotFoundException('Dispute not found');
    if (dispute.status !== 'OPEN') throw new BadRequestException('Cannot add messages to a closed dispute');

    const isAdmin = ([
      UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.ADMIN_SUPPORT, UserRole.ADMIN_MANAGER,
    ] as UserRole[]).includes(role as UserRole);

    const canMessage = userId === dispute.openedByUserId || userId === dispute.targetUserId || isAdmin;
    if (!canMessage) throw new ForbiddenException('You cannot add messages to this dispute');
  ...
  async updatePriority(id: string, priority: string, adminId: string) {
    ...
    if (!validPriorities.includes(priority)) throw new BadRequestException('Invalid priority level');
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.activityLog.create({
        data: {
          ...
          metadata: { disputeId: id, oldPriority: oldDispute.priority, newPriority: priority },
        },
      });
  ...
  // ==================== GET STATISTICS ====================
  async getStats() {
    const [totalOpen, totalResolved, totalRejected, urgentOpen, breachedSLA] = await Promise.all([
