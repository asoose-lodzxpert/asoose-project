import { 
  Controller, Get, Param, Query, Patch, Body, ParseIntPipe, Post, UseGuards,Delete 
} from '@nestjs/common';
...
export class RidersController {
  ...
  async adjustWallet(
    ...
    return this.ridersService.adjustWallet(id, body.type, body.amount, body.reason);
  }

  @Post(':id/payouts')
  @Roles(UserRole.SUPER_ADMIN, UserRole.ADMIN_FINANCE)
  async createPayout(@Param('id') id: string, @Body() body: { amount: number }) {
  ...
  async processPayout(
    ...
    return this.ridersService.processPayout(payoutId, body.status, body.reference);
  }
  @Delete(':id')
