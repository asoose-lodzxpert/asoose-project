...
import { FcmService } from '../libs/fcm/fcm.service'; // Ensure this path is correct
...
export class NotificationFacade {
  ...
  constructor(
    ...
    @InjectQueue('email') private emailQueue: Queue, // [!code ++]
    private readonly fcmService: FcmService,         // [!code ++]
  ...
  async sendToVendor(storeOwnerId: string, title: string, message: string, metadata?: any) {
  ...
  async sendOrderNotifications(
    ...
    await this.sendEmail(customerEmail, 'Order Confirmation', 'customer-order-placed', {
        name: 'Customer', orderId: orderId.slice(0,8).toUpperCase(), storeName, total, items
    });
    ...
    await this.sendEmail(storeOwnerEmail, 'New Order Received', 'vendor-new-order', {
        orderId: orderId.slice(0,8).toUpperCase(), total, items
    });
  }
}