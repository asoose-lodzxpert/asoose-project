...
describe('TripsCommonService - Geofence Validation', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: RedisService, useValue: { getClient: () => ({ incr: jest.fn(), expire: jest.fn() }) } },
  ...
  it('should ACCEPT Maiduguri center (11.8311, 13.151)', async () => {
    await expect(service.validateGeofence(11.8311, 13.151)).resolves.toBeUndefined();
  });

  it('should ACCEPT Maiduguri university area (11.85, 13.10)', async () => {
    await expect(service.validateGeofence(11.85, 13.10)).resolves.toBeUndefined();
  });

  // ── Known Out-of-Zone Coordinates ──

  it('should REJECT Lagos (6.5244, 3.3792) — outside Maiduguri', async () => {
    await expect(service.validateGeofence(6.5244, 3.3792)).rejects.toThrow(BadRequestException);
  });

  it('should REJECT Abuja (9.0579, 7.4951) — outside Maiduguri', async () => {
    await expect(service.validateGeofence(9.0579, 7.4951)).rejects.toThrow(BadRequestException);
  });

  it('should REJECT London (51.5074, -0.1278) — not in Nigeria', async () => {
    await expect(service.validateGeofence(51.5074, -0.1278)).rejects.toThrow(BadRequestException);
  });
  ...
  it('should ACCEPT a point just inside Maiduguri SW corner', async () => {
    await expect(service.validateGeofence(11.71, 13.01)).resolves.toBeUndefined();
  });

  it('should REJECT a point just outside Maiduguri SW corner', async () => {
    await expect(service.validateGeofence(11.69, 12.99)).rejects.toThrow(BadRequestException);
  });

  // ── Invalid Coordinates ──

  it('should REJECT latitude > 90', async () => {
    await expect(service.validateGeofence(91, 3.4)).rejects.toThrow('Invalid coordinates');
  });

  it('should REJECT longitude < -180', async () => {
    await expect(service.validateGeofence(6.5, -181)).rejects.toThrow('Invalid coordinates');
  });
  ...
  it('should ACCEPT any valid coordinate when no zones exist', async () => {
    ...
    await expect(service.validateGeofence(40.7128, -74.006)).resolves.toBeUndefined();
  });

  // ── Caching ──

  it('should cache zones and not re-query DB on second call', async () => {
    await service.validateGeofence(6.5, 3.4);
