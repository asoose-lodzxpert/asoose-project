...
export class RideService {
  ...
  static async createRide(data: any, token?: string) {
    return ApiService.post<{ ride: Ride; payment: any }>("/trips/rides/request", data, token);
  }

  static async confirmRide(rideId: string, paymentMethod: string, token?: string) {
  ...
  static async getDriverLocation(rideId: string, token?: string) {
    return ApiService.get<{ latitude: number; longitude: number }>(`/trips/rides/${rideId}/driver-location`, token);
  }
