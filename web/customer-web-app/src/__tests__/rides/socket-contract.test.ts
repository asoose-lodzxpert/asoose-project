 ...
 *        Instead it sets hard-coded defaults (photoUrl="/profile.jpg", rating=null).
 */
...
describe('C1 — DriverAcceptedEvent type contract', () => {
   ...
   */
  function makeDriverAcceptedEvent(overrides: Partial<DriverAcceptedEvent['driver']> = {}): DriverAcceptedEvent {
    return {
...
describe('C1 — onDriverAccepted handler sets safe defaults', () => {
  ...
  it('sets photoUrl to "/profile.jpg" (not driver.image)', () => {
    const event: DriverAcceptedEvent = {
      ...
      driver: {
        ...
        name: 'Jane',
        phone: '080',
        vehicle: { brand: 'B', model: 'M', plateNumber: 'P', color: 'Black', year: 2021 },
      },
    };
    const result = simulateDriverAcceptedHandler(event);
    expect(result.photoUrl).toBe('/profile.jpg');
  });
  ...
  it('maps vehicle.brand → make, vehicle.model → model, vehicle.plateNumber → licensePlate', () => {
    const event: DriverAcceptedEvent = {
      ...
      driver: {
        ...
        name: 'Bob',
        phone: '090',
        vehicle: { brand: 'Honda', model: 'Civic', plateNumber: 'XYZ-789', color: 'Red', year: 2023 },
      },
    };
    const result = simulateDriverAcceptedHandler(event);
...
describe('C1 — type regression guard', () => {
  it('DriverAcceptedEvent.driver type keys are exactly {id, name, phone, vehicle}', () => {
    ...
    const event: DriverAcceptedEvent = {
      type: 'DRIVER_ACCEPTED',
      rideId: 'r1',
      driver: { id: '1', name: 'N', phone: 'P', vehicle: { brand: 'B', model: 'M', plateNumber: 'P', color: 'C', year: 2020 } },
    };
