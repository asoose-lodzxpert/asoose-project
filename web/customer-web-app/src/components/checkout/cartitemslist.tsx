...
import { CartItem } from '@/app/checkout/types';
...
export const CartItemsList = ({ items, isProcessing, onAdd, onDecrease, onRemove }: CartItemsListProps) => {
  return (
    <section className="bg-white dark:bg-[#151515] p-5 rounded-3xl border border-gray-100 dark:border-white/5 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/" className="text-sm font-bold text-yellow-600 dark:text-yellow-500">
          Add Items
        </Link>
      </div>
          ...
          <div key={item.id} className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex flex-col items-center bg-gray-50 dark:bg-white/5 rounded-lg">
                <button
                  onClick={() => !isProcessing && onAdd({ ...item, quantity: 1 })}
                ...
                <button
                  onClick={() => !isProcessing && onDecrease(item.id)}
