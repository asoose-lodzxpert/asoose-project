...
export const RideCard = ({ id, status, date, total, description }: RideCardProps) => {
  return (
    <div className="flex flex-col sm:flex-row gap-4 p-4 bg-white dark:bg-[#151515] rounded-2xl border border-gray-100 dark:border-white/5 hover:border-yellow-500/30 transition-all cursor-pointer group">
      ...
      <div className="flex-1">
        <div className="flex justify-between items-start mb-2">
          <div>
            <span className="text-xs font-bold text-gray-400">RIDE #{id.slice(0, 8).toUpperCase()}</span>
            <h4 className="font-bold text-base">{description}</h4>
          </div>
          <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${
            status === 'COMPLETED' ? 'bg-green-100 text-green-700 dark:bg-green-500/20 dark:text-green-400' :
