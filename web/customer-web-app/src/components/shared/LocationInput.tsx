...
export function LocationInput({
  ...
  // Google services — PlacesService created with a hidden div (no map needed)
  ...
  // Init Google services once the API is loaded
  useEffect(() => {
  ...
  // Fetch predictions on debounced value
  useEffect(() => {
  ...
  const handleUseCurrentLocation = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const msg = validationError instanceof Error ? validationError.message : 'Invalid geolocation result';
        setGeoError(msg);
        toast.error(msg);
      ...
      <div className="relative flex items-center">
        ...
        <div className="absolute right-2 flex items-center gap-1">
          ...
          {showGeolocation && (
            ...
            >
              {isGeolocating ? <Loader2 size={18} className="animate-spin" /> : <Navigation size={18} />}
            </button>
          )}
        </div>
      </div>
