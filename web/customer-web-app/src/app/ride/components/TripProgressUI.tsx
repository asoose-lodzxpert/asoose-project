...
interface TripProgressUIProps {
  destination: string;
  driverName: string;
  driverImage?: string; // Optional URL
  etaMinutes: number;
}
  ...
  return (
    <div className="h-full flex flex-col justify-between pointer-events-none md:pointer-events-auto font-sans relative">
      ...
      <div className="pointer-events-auto mx-4 mt-4 animate-in slide-in-from-top-10 fade-in duration-500">
        <div className="bg-white rounded-full shadow-lg p-2 pl-6 pr-2 flex items-center justify-between">
          <div className="flex items-baseline gap-2">
             <span className="text-emerald-500 font-bold text-sm">Trip in Progress</span>
             <span className="text-gray-400 text-xs">•</span>
             <span className="text-gray-900 font-bold text-sm">{etaMinutes} mins remaining</span>
          </div>
          ...
          <div className="flex items-center gap-2 bg-gray-100 rounded-full px-3 py-1.5">
             <div className="w-6 h-6 bg-yellow-500 rounded-full flex items-center justify-center text-[10px] font-bold text-white">
                {driverName.charAt(0)}J
             </div>
             <span className="text-xs font-semibold text-gray-700">{driverName}</span>
          </div>
        </div>
      </div>
      ...
      ">
        ...
        <div className="px-6 flex items-center justify-between pb-4">
          <div>
            <h2 className="text-xl font-bold text-gray-900 mb-1">{destination}</h2>
          ...
          {/* Safety Button */}
          <button className="w-12 h-12 bg-red-100 rounded-full flex items-center justify-center shadow-sm hover:bg-red-200 transition active:scale-95 group">
             <ShieldAlert className="text-red-600 fill-red-600/20 group-hover:scale-110 transition" size={24} />
          </button>
        </div>
      </div>
    </div>
