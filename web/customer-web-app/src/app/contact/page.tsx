...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatus({ type: 'success', message: 'Message sent! Our team will get back to you soon.' });
        ...
        <section className="py-20 px-6 max-w-4xl mx-auto">
          <div className="space-y-16">
            <div>
              ...
              <p className="opacity-60">Have questions? Our team is here to help.</p>
            </div>

            <div className="grid md:grid-cols-3 gap-8">
              <ContactInfo icon={<Mail />} label="Email" value="support@asoose.com" href="mailto:support@asoose.com" />
              <ContactInfo icon={<Phone />} label="Phone" value="+234 (0) 8061966145" href="tel:+2348061966145" />
              <div className="flex gap-4 items-start">
                <MapPin className="text-yellow-500 shrink-0" size={20} />
                <div>
                  <div className="text-sm opacity-60">Address</div>
                  <div className="text-sm">Suite B17 Musty Global Plaza, Old GRA, Maiduguri</div>
                </div>
              </div>
            </div>

            <div className={`p-8 rounded-lg border ${darkMode ? 'border-white/10' : 'border-black/10'}`}>
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs opacity-60 block mb-2">Name</label>
                    <input 
                      required
                      name="name"
                      value={formData.name}
                  ...
                  <div>
                    <label className="text-xs opacity-60 block mb-2">Email</label>
                    <input 
                      required
                      name="email"
                ...
                <div>
                  <label className="text-xs opacity-60 block mb-2">Subject</label>
                  <select 
                    name="subject"
                ...
                <div>
                  <label className="text-xs opacity-60 block mb-2">Message</label>
                  <textarea 
                    required
                    name="message"
  ...
  return (
    <div className="flex gap-4 items-center">
      ...
      <div>
        ...
        <a href={href} className="text-yellow-500 hover:underline font-medium break-all">{value}</a>
      </div>
    </div>
  );
}