  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] text-gray-900 dark:text-gray-100 font-sans pb-24 transition-colors duration-300">
      ...
      <main className="max-w-7xl mx-auto min-h-[80vh]">
            ...
            <div className="flex justify-between items-end mb-2">
              <div>
                ...
                <p className="text-gray-500 dark:text-gray-400 font-medium">
                  {data.vendors.length}{" "}
                  {data.vendors.length === 1 ? "store" : "stores"} near you
                </p>
              </div>
            </div>
          </div>
        ...
        {/* --- STICKY FILTER BAR --- */}
        <div
          className={`sticky top-[0px] z-20 py-3 px-4 mb-6 transition-all duration-300 ${
            scrolled
        ...
        >
          <div className="flex items-center gap-2 max-w-7xl mx-auto">
            <button className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-[#151515] border border-gray-200 dark:border-white/10 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors shrink-0">
              ...
              <span className="sr-only sm:not-sr-only text-sm font-bold">
                Filters
              </span>
            </button>
              ...
              {Object.entries(filters).map(([label, slug]) => {
                ...
                return (
                  ...
                  >
                    <span
                      className={
                        isPending && isActive ? "opacity-0" : "opacity-100"
                      }
                    >
                      {label}
                    </span>
        ...
        <div className="px-4">
          <div
            className={`transition-all duration-500 ${isPending ? "opacity-50 scale-[0.99] grayscale-[0.5]" : "opacity-100 scale-100"}`}
          >
            {data.vendors.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {data.vendors.map((vendor, index) => (
                  ...
                  >
                    {/* ✅ Correct Link with /main prefix */}
                    <Link
                      href={`/main/store/${vendor.slug || vendor.id}`}
              ...
              <div className="flex flex-col items-center justify-center py-32 text-center">
                ...
                <p className="text-gray-500 max-w-sm mx-auto mb-6">
                  We couldn't find any stores matching "{activeFilter}" in this
                  category.
                </p>
                {activeFilter !== "all" && (
                  <button
