...
export default function RideSelector({
  ...
  const getRideIcon = (type: RideType) => {
    if (type.icon)
      return <img src={type.icon} alt={type.displayName} className="w-8 h-8" />;
      ...
      <div className="flex-1 overflow-y-auto p-4 no-scrollbar">
        {isSelectingPayment ? (
          <div className="animate-in slide-in-from-right-10">
            <button
              onClick={() => setIsSelectingPayment(false)}
              className="flex items-center gap-2 mb-4 text-gray-600 dark:text-zinc-400"
            >
              <ChevronLeft size={20} />{" "}
              <span className="font-bold">Back to rides</span>
            </button>
            ...
            {availableRideTypes.length === 0 ? (
              ...
              <div className="space-y-3">
                {availableRideTypes.map((type) => {
                  ...
                  const estimate = priceEstimates
                    ? priceEstimates[type.id]
                    : null;
            ...
            {priceEstimates &&
              ...
              priceEstimates[selectedRideId] && (
                <div className="mt-4">
                  ...
                  >
                    {showBreakdown ? "Hide Details" : "Show fare breakdown"}{" "}
                    <ChevronDown
                      size={14}
                  ...
                  {showBreakdown && (
                    <FareBreakdown
                      ...
                      rideType={
                        availableRideTypes.find((t) => t.id === selectedRideId)
                          ?.displayName || selectedRideId
                      }
                    />
                  )}
                </div>
      ...
      <div className="p-4 border-t border-gray-100 dark:border-zinc-800 bg-white dark:bg-[#0a0a0a]">
        <div className="flex items-center justify-between mb-4">
          <button
            ...
            className="flex items-center gap-2 text-sm font-semibold text-gray-700 dark:text-zinc-300"
          >
            {getIcon(selectedPayment.type)} {selectedPayment.label}{" "}
        ...
        >
          {isRequesting ? (
            ...
            `Confirm ${availableRideTypes.find((t) => t.id === selectedRideId)?.displayName || "Ride"}`
          )}
        </button>
      </div>
    </div>
