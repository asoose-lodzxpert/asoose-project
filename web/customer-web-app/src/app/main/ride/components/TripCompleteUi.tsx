    ...
    <div className="h-full flex flex-col bg-white dark:bg-[#0a0a0a] overflow-y-auto no-scrollbar transition-colors duration-300">
      ...
      {step === 'SUPPORT' && (
        <div className="p-6 animate-in slide-in-from-right-10">
          <div className="flex items-center justify-between mb-6">
            <h3 className="text-xl font-bold text-gray-900 dark:text-white">Help Center</h3>
            <button onClick={() => setStep('RATING')} className="hover:bg-gray-100 dark:hover:bg-zinc-900 p-1 rounded-full transition-colors">
              <X size={24} className="text-gray-400 dark:text-zinc-500" />
            </button>
          </div>
          <p className="text-sm text-gray-500 dark:text-zinc-500 mb-4 font-medium uppercase tracking-wider">Common Issues</p>
          <div className="space-y-2">
            {commonIssues.map((issue, i) => (
              ...
              >
                <span className="text-sm font-bold text-gray-700 dark:text-zinc-300">{issue}</span>
                <MessageCircle size={18} className="text-gray-300 dark:text-zinc-600 group-hover:text-emerald-500 transition-colors" />
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 'FINISHED' && (
        <div className="h-full flex flex-col items-center justify-center p-6 text-center">
           ...
           <p className="text-gray-500 dark:text-zinc-400 mb-8 leading-relaxed">Your feedback helps us keep the community safe and reliable.</p>
           <button 
             onClick={onClose} 
