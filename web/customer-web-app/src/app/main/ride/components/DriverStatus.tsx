      ...
      {status === 'ON_WAY' && (
        <div className="pointer-events-auto mx-4 mt-4 animate-in slide-in-from-top-10 fade-in duration-500">
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-xl p-4 flex items-center justify-between border dark:border-zinc-800">
            <div>
              <h2 className="text-lg font-bold text-gray-900 dark:text-white">Driver is on the way</h2>
              <p className="text-emerald-600 dark:text-emerald-400 font-semibold text-sm">Arriving in {etaMinutes} minutes</p>
            </div>
        ...
        <div className="px-6">
          {status === 'ARRIVED' && (
            <div className="text-center mb-6 animate-in fade-in zoom-in duration-300">
              <h2 className="text-2xl font-bold text-emerald-500 dark:text-emerald-400 mb-1">Driver has arrived!</h2>
              <p className="text-gray-500 dark:text-zinc-500 text-sm">Your ride is ready</p>
            </div>
          )}
            ...
            <div className="flex-1">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1">
                   ...
                   <span className="text-xs font-bold text-gray-700 dark:text-zinc-300">{driver.rating}</span>
                   <span className="text-xs text-gray-400 dark:text-zinc-500">({driver.trips} trips)</span>
                </div>
              </div>
              
              <div className="text-sm font-medium text-gray-600 dark:text-zinc-400 mt-1">
                {driver.vehicle} <span className="mx-1">•</span> <span className="font-bold text-gray-900 dark:text-zinc-200">{driver.plate || 'KJA-123'}</span>
              </div>
            </div>
          </div>
          ...
          {status === 'ARRIVED' ? (
            <div className="bg-gray-50 dark:bg-zinc-900/50 rounded-xl p-4 flex items-center gap-3 mb-4 border dark:border-zinc-800">
              ...
              <p className="text-sm font-semibold text-gray-700 dark:text-zinc-300">
                Driver is waiting at <span className="text-gray-900 dark:text-white">{tripDetails.pickup.split(',')[0]}</span>
              </p>
            </div>
          ) : (
            <div className="border-t border-gray-100 dark:border-zinc-800 pt-4">
               <div className="flex items-center justify-between mb-4">
                <span className="font-bold text-gray-900 dark:text-white">Trip Details</span>
                <ChevronDown size={18} className="text-gray-400 dark:text-zinc-600" />
              </div>
              <div className="flex flex-col gap-4 relative pl-2">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm font-medium text-gray-600 dark:text-zinc-400 truncate">{tripDetails.pickup}</span>
                </div>
                <div className="flex items-center gap-3">
                  <div className="w-5 h-5 rounded-full bg-white dark:bg-zinc-900 border-[5px] border-red-500 z-10" />
                  <span className="text-sm font-medium text-gray-600 dark:text-zinc-400 truncate">{tripDetails.dropoff}</span>
                </div>
              </div>
            </div>
          )}
