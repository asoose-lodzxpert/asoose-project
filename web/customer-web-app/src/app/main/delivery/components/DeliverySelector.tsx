...
import { Truck, Bike, Car, Info, Scale, ShoppingBag, FileText } from 'lucide-react';
...
export default function DeliverySelector({ onConfirm, basePrice }: DeliverySelectorProps) {
    ...
    <div className="p-6 h-full flex flex-col">
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-4">
                  <div className={`p-3 rounded-xl transition-colors ${isSelected ? 'bg-yellow-500 text-white' : 'bg-gray-100 dark:bg-zinc-800 text-gray-400 group-hover:bg-gray-200'}`}>
                    <v.icon size={24} />
                  </div>
                  <div>
              ...
              {/* Capacity & Usage Info */}
              <div className={`mt-2 pt-2 border-t flex gap-4 text-xs ${isSelected ? 'border-yellow-200 dark:border-yellow-800/30' : 'border-gray-100 dark:border-zinc-800'}`}>
      ...
      <div className="pt-6 space-y-4 border-t border-gray-100 dark:border-zinc-800 mt-4">
        <div className="flex items-start gap-2 text-xs text-gray-400 bg-gray-50 dark:bg-zinc-900 p-3 rounded-lg">
          ...
          <p>Final price varies by vehicle type. All trips include goods-in-transit insurance.</p>
        </div>
        <button
        ...
        >
          <span>Request {vehicles.find(v => v.id === selectedId)?.label}</span>
