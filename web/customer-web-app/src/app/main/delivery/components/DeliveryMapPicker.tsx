...
export function DeliveryMapPicker({
  ...
  const defaultCenter = useMemo(
    () =>
      ...
      (selectedCity?.latitude != null && selectedCity?.longitude != null
        ...
        : fallbackCenter),
    [initialPosition, selectedCity, userLocation],
  );
  ...
  const pinLocation = useCallback(async (next: Position) => {
    ...
    try {
      ...
      setAddress(
        result.address || `${next.lat.toFixed(5)}, ${next.lng.toFixed(5)}`,
      );
    } catch {
      if (current !== requestId.current) return;
  ...
  const useCurrentLocation = async () => {
    ...
    try {
      ...
      setError(
        "Current location is unavailable. Tap the map to place the pin instead.",
      );
    } finally {
      setResolving(false);
    }
  ...
  return createPortal(
    <div
      ref={dialog}
      className="fixed inset-0 z-[100] flex items-end bg-black/60 backdrop-blur-sm sm:items-center sm:justify-center sm:p-5"
      role="dialog"
      aria-modal="true"
      aria-label={`Choose ${kind} on map`}
    >
      <div className="flex h-[88dvh] w-full flex-col overflow-hidden rounded-t-[2rem] bg-white shadow-2xl dark:bg-[#151515] sm:h-[min(760px,88vh)] sm:max-w-3xl sm:rounded-[2rem]">
        <header className="flex items-center justify-between border-b border-black/5 px-4 py-4 dark:border-white/10 sm:px-6">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-full bg-gray-100 p-2.5 dark:bg-white/5"
        ...
        <div className="relative min-h-0 flex-1 bg-gray-100 dark:bg-zinc-900">
          {loadError ? (
            <div className="flex h-full items-center justify-center p-8 text-center text-sm font-bold text-red-500">
              Google Maps could not load. Check the Maps API configuration.
            </div>
          ) : !isLoaded ? (
          ...
          <div className="pointer-events-none absolute left-1/2 top-4 -translate-x-1/2 rounded-full bg-zinc-950/90 px-4 py-2 text-[11px] font-bold text-white shadow-lg backdrop-blur">
          ...
          <button
            ...
            disabled={resolving}
            className="absolute bottom-4 right-4 flex items-center gap-2 rounded-2xl bg-white px-4 py-3 text-xs font-black text-zinc-950 shadow-xl disabled:opacity-60 dark:bg-zinc-900 dark:text-white"
          >
            <Crosshair className="h-4 w-4 text-blue-500" /> My location
          </button>
        </div>
        ...
        <footer className="border-t border-black/5 p-4 dark:border-white/10 sm:p-6">
          ...
          <div className="mb-4 flex items-start gap-3 rounded-2xl bg-gray-50 p-3 dark:bg-white/[0.04]">
            <span
              className={`mt-0.5 rounded-xl p-2 ${kind === "pickup" ? "bg-yellow-100 text-yellow-700" : "bg-blue-100 text-blue-700"}`}
            >
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-sm font-bold leading-5 dark:text-white">
                {resolving
                  ? "Finding the address…"
            ...
            {resolving && (
              <Loader2 className="mt-2 h-4 w-4 animate-spin text-yellow-500" />
            )}
          </div>
          <button
            type="button"
            ...
            className="flex w-full items-center justify-center gap-2 rounded-2xl bg-yellow-400 py-4 text-sm font-black text-black transition hover:bg-yellow-300 disabled:cursor-not-allowed disabled:opacity-50"
          >
