...
import { 
  ...
  Loader2, User, Phone, AlertCircle, RefreshCw
} from 'lucide-react';
...
const usePaymentPolling = () => {
  ...
  const pollPaymentStatus = useCallback(async (
    ...
    const poll = async () => {
      try {
        const success = await DeliveryService.pollDeliveryStatus(deliveryId, 'REQUESTED');
...
export default function DeliveryPage() {
  ...
  useEffect(() => {
    const checkPendingDelivery = async () => {
      ...
      if (hasPending && activeDeliveryId && stage === DeliveryStage.PAYMENT_PENDING) {
        try {
          const success = await DeliveryService.pollDeliveryStatus(activeDeliveryId, 'REQUESTED');
  ...
  const handleInitializeDelivery = async () => {
    ...
    try {
      ...
      const deliveryRes = await DeliveryService.createDelivery({
        ...
        packageDetails: sanitizeInput(`${packageInfo.type} - ${packageInfo.instructions || 'No special instructions'}`, 500),
      ...
      toast.error(error.response?.data?.message || "Failed to process delivery. Please try again.");
      setApiError(true);
  ...
  const startPaymentMonitoring = useCallback(() => {
    ...
    pollPaymentStatus(
      ...
      () => {
        toast.warning("Payment validation timed out. Please check your payment status.");
      }
    );
  ...
  const handleReviewSubmit = async (rating: number, comment: string) => {
    ...
    try {
      await DeliveryService.rateDelivery(activeDeliveryId, rating, sanitizeInput(comment, 1000));
  ...
  const renderStageContent = () => {
    ...
    switch (stage) {
        ...
        return (
          <div className="flex flex-col items-center justify-center py-20 gap-4" role="status" aria-live="polite">
            ...
            <div className="text-center">
              ...
              <p className="text-sm text-gray-500">Calculating best route & delivery fee...</p>
            </div>
          </div>
        );
      ...
      case DeliveryStage.PAYMENT_PENDING:
        return (
          <div className="text-center py-20" role="status" aria-live="polite">
            <Loader2 size={48} className="animate-spin text-blue-500 mx-auto mb-4" aria-hidden="true" />
            <h3 className="text-xl font-bold">Waiting for Payment</h3>
            <p className="text-sm text-gray-500 mt-2">You have been redirected to the payment gateway.</p>
            <p className="text-xs text-gray-400 mt-1">We will automatically update when payment is confirmed.</p>
            <button 
      ...
      case DeliveryStage.FINDING_COURIER:
        return (
          <div className="max-w-md mx-auto py-24 text-center space-y-8 animate-in fade-in zoom-in-95" role="status" aria-live="polite">
        ...
        return (
          <div className="text-center py-20">
            <div className="mb-6">
              ...
              <p className="text-gray-500 mt-2">Your package has been successfully delivered</p>
            </div>
            <button 
              ...
              <section>
                <div className="relative space-y-8 mt-10">
                  <div className="border-b border-zinc-200 dark:border-zinc-800 pb-2">
                    <label htmlFor="pickup-location" className="text-sm font-medium mb-2 block text-zinc-500">
                      Pickup Location
                    </label>
                  ...
                  <div className="border-b border-zinc-200 dark:border-zinc-800 pb-2">
                    <label htmlFor="dropoff-location" className="text-sm font-medium mb-2 block text-zinc-500">
                      Delivery Address
                    </label>
                    <LocationAutocomplete 
            ...
            <div className="lg:col-span-5 space-y-12">
              ...
              <div className="pt-8 border-t border-zinc-200 dark:border-zinc-800">
                <button 
                  ...
                  aria-label={isFormValid ? "Proceed to calculate delivery price" : "Complete all required fields to continue"}
                >
                  <span className="font-medium">
                    {isFormValid ? "Calculate Price & Proceed" : "Complete All Fields"}
                  </span>
...
const DetailInput: React.FC<DetailInputProps> = ({ 
  ...
  <div className={`border-b pb-2 transition-colors ${
    ...
    <label className={`flex items-center gap-2 text-sm font-medium mb-2 ${
      ...
      <Icon size={16} className={error ? 'text-red-500' : 'text-yellow-500/50'} /> 
      {label}
    </label>
    <input 
      type="text"
    ...
    />
    {error && (
      <p id={`${label}-error`} className="text-xs text-red-500 mt-1" role="alert">
        {error}
      </p>
    )}
  </div>
