...
import { ChevronLeft, Loader2, MessageSquare, AlertCircle, ShoppingBag, MapPin, Clock } from 'lucide-react';
...
export default function OrderDetailsPage() {
  ...
  const { data: order, error, isLoading, mutate } = useSWR(shouldFetch, fetcher, {
      refreshInterval: (data) => data && ['DELIVERED', 'CANCELLED', 'REJECTED'].includes(data.status) ? 0 : 5000,
  ...
  if (error || !order) return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 dark:bg-[#0a0a0a] p-6 text-center">
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6 max-w-xs mx-auto">{error?.message || "We couldn't locate the order details you requested."}</p>
      <Link href="/main/orders" className="px-6 py-3 bg-yellow-500 text-black font-bold rounded-xl hover:bg-yellow-400 transition-colors">
        Back to Orders
      </Link>
    </div>
  );
      ...
      <div className="sticky top-0 z-20 bg-gray-50/80 dark:bg-[#0a0a0a]/80 backdrop-blur-md px-4 py-4 border-b border-gray-200 dark:border-white/5">
         <div className="max-w-2xl mx-auto flex items-center gap-4">
            ...
            <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 dark:text-gray-400 font-mono">ID: #{order.id.split('-')[0].toUpperCase()}</p>
            </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-[#151515] rounded-3xl p-6 border border-gray-100 dark:border-white/5 shadow-sm">
           <div className="mb-8 text-center">
              ...
              <div className="flex items-center justify-center gap-2 text-sm text-gray-500 dark:text-gray-400">
                 ...
                 {order.status === 'DELIVERED' 
                   ? <span>Arrived on {new Date(order.deliveredAt).toLocaleDateString()}</span>
                   : <span>Estimated Arrival: {order.estimatedArrival || 'Calculating...'}</span>
                 }
              </div>
           </div>
        ...
        {hasDispute && (
          <div className="p-4 bg-red-50 dark:bg-red-900/10 rounded-2xl flex items-start gap-3 border border-red-100 dark:border-red-900/20">
            ...
            <div>
              <p className="font-bold text-red-700 dark:text-red-400 text-sm">Dispute Open</p>
              <p className="text-xs text-red-600 dark:text-red-300/80 mt-1 capitalize">Status: {order.dispute.status}</p>
            </div>
          </div>
        )}
        ...
        <div className="bg-white dark:bg-[#151515] rounded-3xl p-6 border border-gray-100 dark:border-white/5 shadow-sm space-y-6">
           ...
           <div className="space-y-4">
             {order.items.map((item: any) => (
               <div key={item.id} className="flex justify-between items-start py-2 border-b border-gray-50 dark:border-white/5 last:border-0 last:pb-0">
        ...
        <div className="bg-white dark:bg-[#151515] rounded-3xl p-6 border border-gray-100 dark:border-white/5 shadow-sm">
            ...
            <div className="pl-2 border-l-2 border-gray-100 dark:border-white/10 ml-2 space-y-6">
                <div className="relative pl-6">
                    ...
                    <p className="text-sm font-bold">{order.store?.name || 'Store'}</p>
                ...
                <div className="relative pl-6">
                    ...
                    <p className="text-sm font-bold">{order.addressDetails?.address || 'Pickup'}</p>
        ...
        <div className="pt-4 space-y-3">
           ...
           {isExpired && (
             <div className="text-center">
                 <p className="text-xs text-gray-400 uppercase tracking-widest font-bold">Dispute window closed</p>
             </div>
           )}
        </div>
