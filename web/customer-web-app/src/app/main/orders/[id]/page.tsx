  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0a0a0a] flex flex-col items-center p-4 pb-24 transition-colors duration-300">
      ...
      <div className="w-full max-w-md mb-6 flex items-center justify-between">
         <button onClick={() => router.back()} className="p-3 bg-white dark:bg-white/5 rounded-full shadow-sm">
            <ChevronLeft className="w-5 h-5" />
         </button>
         <div className="text-right">
            <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest">Order Reference</p>
            <p className="text-xs font-bold font-mono">#{order.id.split('-')[0].toUpperCase()}</p>
         </div>
      </div>
        ...
        {hasDispute && (
          <div className="bg-blue-500/10 border border-blue-500/20 p-5 rounded-[2rem] flex items-start gap-4 animate-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex-1">
              <p className="text-sm font-black text-blue-600 dark:text-blue-400">Issue Under Review</p>
        ...
        <div className="bg-white dark:bg-[#151515] rounded-[2.5rem] p-6 shadow-sm border border-gray-100 dark:border-white/5 space-y-6">
           <div className="space-y-4">
              <div className="flex justify-between items-center">
                <h3 className="font-black text-xs uppercase tracking-widest text-gray-400">Order Items</h3>
           ...
           <div className="pt-4 border-t border-dashed border-gray-200 dark:border-white/10 flex justify-between items-center">
              <span className="text-sm font-black text-gray-400">GRAND TOTAL</span>
              <span className="text-xl font-black text-yellow-500 tracking-tighter">₦{order.total.toLocaleString()}</span>
           </div>
           ...
           <div className="flex items-start gap-3 p-4 bg-gray-50 dark:bg-white/5 rounded-3xl">
              ...
              <div>
                 ...
                 <p className="text-xs font-bold line-clamp-1">{order.addressDetails?.address}</p>
              </div>
           </div>
        </div>
