    ...
    <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            <span className="relative flex h-2.5 w-2.5">
              ...
              <span
                className={`relative inline-flex rounded-full h-2.5 w-2.5 ${status === "CANCELLED" ? "bg-red-500" : isLive ? "bg-emerald-500" : "bg-gray-400"}`}
              ></span>
            </span>
      ...
      <div className="relative space-y-0 pl-2">
        ...
        {timeline.map((step, idx) => {
          ...
          const isCurrent =
            !isCompleted && (idx === 0 || timeline[idx - 1]?.time !== null);
          const isPending = !isCompleted && !isCurrent;

          return (
            <div
              key={idx}
              className={`relative flex gap-4 pb-8 last:pb-0 group ${isPending ? "opacity-50" : "opacity-100"}`}
            >
              {/* Timeline Node */}
              <div
                className={`relative z-10 w-10 h-10 rounded-full border-4 flex items-center justify-center bg-white transition-all duration-300
                ${
                  isCompleted
              ...
              <div className="pt-1 flex-1">
                <div className="flex justify-between items-start">
                  <div>
                    <p
                      className={`font-bold text-base ${isCompleted || isCurrent ? "text-gray-900" : "text-gray-500"}`}
                    >
                      {step.label}
                    </p>
                    {step.description && (
