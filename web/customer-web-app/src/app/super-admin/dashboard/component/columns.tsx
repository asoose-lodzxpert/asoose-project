...
import { Clock, ShoppingCart, Car, Package, Truck, Users, ExternalLink, AlertCircle, Eye, MoreHorizontal, ShieldCheck, FileText } from 'lucide-react';
...
export const getActivityIcon = (type: string) => {
  switch (type?.toLowerCase()) {
    ...
    case 'ride': return <Car className="w-4 h-4 text-orange-400" />;
    case 'vendor': return <Package className="w-4 h-4 text-purple-400" />;
    case 'delivery': return <Truck className="w-4 h-4 text-indigo-400" />;
    case 'customer': return <Users className="w-4 h-4 text-green-400" />;
  ...
  return (
    <div className="bg-[#1E293B] border border-gray-800 rounded-lg p-4 mb-3">
      <div className="flex justify-between items-start mb-3">
        ...
        <span className={`px-2 py-1 rounded text-xs uppercase border border-gray-700 bg-gray-800 text-gray-300`}>{activity.type}</span>
      </div>
      <div className="flex items-start gap-3 mb-4 text-sm">
        <div className="p-2 bg-gray-800/50 rounded-lg mt-0.5 border border-gray-700">{getActivityIcon(activity.type)}</div>
        ...
        {isClickable ? (
          <Link href={link} className="flex-1">
            <button className="flex items-center gap-2 text-yellow-500 hover:text-yellow-400 font-bold text-xs transition-colors">
              <ExternalLink className="w-3 h-3" /> {activity.action || 'View Details'}
            </button>
          </Link>
        ) : (
          <span className="text-xs text-gray-600 italic">No details available</span>
        )}
...
export const renderAlertMobileCard = (alert: Alert) => {
  const link = getActivityLink({ ...alert, entityId: alert.entityId, type: alert.entityType } as any);
  ...
  return (
    <div className="bg-[#1E293B] border border-gray-800 rounded-lg p-4 mb-3">
      ...
      <div className="flex items-start gap-3 mb-4 text-sm">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-2">
            <span className="text-gray-400 text-xs bg-gray-800 px-1.5 py-0.5 rounded">Cat: {alert.category}</span>
            <span className="text-gray-500 text-xs">•</span>
            <span className="text-gray-400 text-xs font-mono">{formatDistanceToNow(parseISO(alert.time), { addSuffix: true })}</span>
          </div>
        </div>
      </div>
      <div className="flex gap-3 pt-3 border-t border-gray-800">
        ...
        <button className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-bold ${alert.status === 'New' ? 'bg-yellow-500 text-black hover:bg-yellow-400' : 'bg-green-500 text-black hover:bg-green-400'}`}>
