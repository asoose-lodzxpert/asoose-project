                    ...
                    <div key={admin.id} className="flex items-center justify-between p-4 bg-[#0F172A] border border-gray-800 rounded-2xl group hover:border-blue-500/50 transition-all">
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {admin.role !== 'SUPER_ADMIN' && (
                          <button onClick={() => handleDeleteAdmin(admin.id)} className="p-2 text-gray-500 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-all">
                    ...
                    <div key={def.key} className="bg-[#1E293B] border border-gray-800 rounded-3xl p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 shadow-xl">
                      ...
                      <div className="min-w-[180px]">
                        {def.type === 'toggle' ? (
                          ...
                          >
                            <span className={`absolute left-1 top-1 bg-white w-4 h-4 rounded-full transition-transform ${currentValue ? 'translate-x-6' : 'translate-x-0'}`} />
                          </button>
                        ) : (
          ...
          <div className="bg-[#1E293B] border border-gray-700 rounded-3xl w-full max-w-md shadow-2xl p-8">
            ...
            <form onSubmit={handleCreateAdmin} className="space-y-5">
              <div className="space-y-2">
                ...
                <input required type="text" className="w-full bg-[#0F172A] border border-gray-700 rounded-xl p-3 text-white text-sm focus:border-blue-500 outline-none" placeholder="John Doe" value={newAdmin.name} onChange={e => setNewAdmin({...newAdmin, name: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Work Email</label>
                <input required type="email" className="w-full bg-[#0F172A] border border-gray-700 rounded-xl p-3 text-white text-sm focus:border-blue-500 outline-none" placeholder="john@asoose.com" value={newAdmin.email} onChange={e => setNewAdmin({...newAdmin, email: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Temp Password</label>
                <input required type="password" minLength={8} className="w-full bg-[#0F172A] border border-gray-700 rounded-xl p-3 text-white text-sm focus:border-blue-500 outline-none" placeholder="••••••••" value={newAdmin.password} onChange={e => setNewAdmin({...newAdmin, password: e.target.value})} />
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Assign Permission</label>
                <select className="w-full bg-[#0F172A] border border-gray-700 rounded-xl p-3 text-white text-sm focus:border-blue-500 outline-none cursor-pointer" value={newAdmin.role} onChange={e => setNewAdmin({...newAdmin, role: e.target.value})}>
                  {ROLES.map(r => <option key={r.value} value={r.value}>{r.label}</option>)}
                </select>
              </div>
              <div className="flex gap-3 pt-6">
                <button type="button" onClick={() => setIsTeamModalOpen(false)} className="flex-1 py-3 bg-slate-800 text-white rounded-xl font-black text-sm">Cancel</button>
                <button type="submit" disabled={isCreatingAdmin} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-black text-sm disabled:opacity-50">
                  {isCreatingAdmin ? <Loader2 size={18} className="animate-spin mx-auto"/> : 'Create Admin'}
                </button>
              </div>
            </form>
          </div>
