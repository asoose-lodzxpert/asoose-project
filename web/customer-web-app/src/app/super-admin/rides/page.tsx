...
import { 
  Search, Filter, Clock, User, Car, Download, 
  Settings, Eye, AlertTriangle, Loader2, Calendar, XCircle, RefreshCw 
} from 'lucide-react';
...
export default function RidesPage() {
  ...
  const { 
    ...
    isLoading, 
    mutate 
  } = useSWR<RidesApiResponse>(
    `/super-admin/rides?${queryString}`,
    fetcher,
  ...
  // --- Actions ---
  const handleCancelRide = useCallback(async (id: string) => {
  ...
  const getStatusColor = useCallback((status: string) => {
    ...
    if (s === 'COMPLETED') return 'bg-green-500/20 text-green-500 border-green-500/20';
    if (s === 'IN PROGRESS' || s === 'INPROGRESS') return 'bg-blue-500/20 text-blue-400 border-blue-500/20';
    if (s === 'SEARCHING' || s === 'REQUESTED') return 'bg-yellow-500/20 text-yellow-500 border-yellow-500/20';
    if (s === 'CANCELLED') return 'bg-red-500/20 text-red-500 border-red-500/20';
  ...
  const columns = useMemo<ColumnDef<Ride>[]>(() => [
      ...
      cell: ({ row }) => (
        <div>
          ...
          <span className="text-xs text-gray-500 flex items-center gap-1 mt-1">
            ...
            {new Date(row.original.time).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
          </span>
        </div>
      ),
    },
      ...
      cell: ({ row }) => (
        row.original.driver ? (
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium text-sm">{row.original.driver.name}</p>
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-2 text-xs text-gray-400">
            ...
            <span className="truncate max-w-[120px]" title={row.original.from}>
              {row.original.from}
            </span>
          </div>
    ...
    },
    {
      accessorKey: 'fare',
      header: 'Fare',
      cell: ({ row }) => <span className="font-bold text-white text-sm">{row.original.fare}</span>,
    },
    {
      id: 'actions',
      header: 'Actions',
    ...
    <div className="min-h-screen bg-[#0F172A] p-4 md:p-6 pb-20">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-bold text-white">Ride Dispatch</h1>
            <p className="text-gray-400 text-sm mt-1">Real-time monitoring of fleet and trips</p>
          </div>
        ...
        {/* Filters Panel */}
        <div className={`bg-[#1E293B] p-4 rounded-xl border border-gray-800 space-y-4 ${filterOpen ? 'block' : 'hidden md:block'}`}>
          ...
          <div className="bg-[#1E293B] border border-gray-800 rounded-xl overflow-hidden min-h-[400px]">
            {rides.length > 0 ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 text-gray-500">
                  <p className="text-base mb-2">No rides found matching your filters.</p>
                  <button 
                    onClick={handleClearFilters} 
