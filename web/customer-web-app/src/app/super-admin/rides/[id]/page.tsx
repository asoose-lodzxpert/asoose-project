...
export default function RideDetailPage() {
  ...
  const { data: ride, error, isLoading, mutate } = useSWR<RideDetail>(
    ...
    fetcher,
    {
        refreshInterval: 5000, // Poll every 5s for live GPS/Status updates
  ...
  const handleCancelRide = async () => {
    const result = await Swal.fire({
      title: 'Cancel Ride?',
      text: "This will immediately stop the trip. The driver will be notified.",
      icon: 'warning',
  ...
  // Status Check Logic
  const isActive = ['IN_PROGRESS', 'REQUESTED', 'SEARCHING', 'IN PROGRESS', 'In Progress', 'Requested', 'Searching']
    .includes(ride.status?.toUpperCase().replace('_', ' '));
      ...
      <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 pb-20">
          ...
          <div>
             ...
             <div className="flex items-center gap-3">
                ...
                <span className={`px-3 py-1 rounded-full text-sm font-bold uppercase ${
                   ride.status?.toUpperCase().includes('PROGRESS') ? 'bg-blue-500/10 text-blue-500 border border-blue-500/20' : 
                   ride.status?.toUpperCase() === 'COMPLETED' ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 
                   ride.status?.toUpperCase() === 'CANCELLED' ? 'bg-red-500/10 text-red-500 border border-red-500/20' :
          ...
          <div className="flex gap-3">
             {isActive && (
               <button 
                  ...
                  className="px-4 py-2 bg-red-500/10 text-red-500 border border-red-500/20 rounded-lg hover:bg-red-500 hover:text-white transition-colors flex items-center gap-2 font-bold text-sm"
               >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                   ...
                   {ride.timeline.map((step, i) => (
                      <div key={i} className="relative">
                         ...
                         <div className="flex justify-between items-start">
                            <div>
                               <h4 className={`text-sm font-bold ${step.active ? 'text-blue-400' : step.done ? 'text-gray-200' : 'text-gray-500'}`}>
                                  {step.status}
                               </h4>
                            </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm text-left">
                    ...
                    <tbody className="divide-y divide-gray-700">
                      {ride.logs && ride.logs.length > 0 ? ride.logs.map((log, i) => (
                        ...
                        <tr>
                          <td colSpan={3} className="px-4 py-6 text-center text-gray-500 italic">
                            No admin actions recorded for this ride.
                          </td>
                        </tr>
                      )}
                    </tbody>
          ...
          <div className="lg:col-span-1 space-y-6">
                    ...
                    <>
                      <div className="flex items-center gap-4 mb-4">
                          ...
                          <div>
                              ...
                              <div className="flex items-center gap-1 text-yellow-500 text-xs">
                                  <Star className="w-3 h-3 fill-yellow-500" /> {ride.driver.rating}
                              </div>
                          </div>
                      </div>
                      <div className="space-y-2 text-sm bg-[#0F172A] p-3 rounded-xl">
                          <div className="flex justify-between">
                              ...
                              <span className="text-white font-medium">{ride.driver.vehicle}</span>
                          </div>
                          <div className="flex justify-between">
             ...
             <div className="bg-[#1E293B] border border-gray-800 rounded-2xl p-6">
                <div className="flex justify-between items-center mb-4">
                   <h3 className="text-sm font-bold text-gray-400 uppercase">Passenger</h3>
                </div>
                <div className="flex items-center gap-4 mb-4">
                   ...
                   <div>
                      ...
                      <div className="flex items-center gap-1 text-yellow-500 text-xs">
                          <Star className="w-3 h-3 fill-yellow-500" /> {ride.passenger.rating}
                      </div>
                   </div>
                </div>
             ...
             <div className="bg-[#1E293B] border border-gray-800 rounded-2xl p-6">
                ...
                <div className="space-y-2 text-sm">
                   ...
                   <div className="border-t border-gray-700 pt-3 mt-2 flex justify-between items-center">
                      ...
                      <span className="text-yellow-500 font-black text-xl">{ride.fare.total}</span>
                   </div>
                </div>
