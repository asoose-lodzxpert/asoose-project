...
import { X, Search, Bike, CheckCircle, AlertCircle, Loader2 } from 'lucide-react';
...
export default function DriverSelectorModal({ isOpen, onClose, onAssign }: Props) {
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2 min-h-[300px]">
          {isLoading ? (
            <div className="flex flex-col items-center justify-center h-full py-8 text-blue-500 gap-2">
                ...
                <span className="text-sm font-medium text-gray-400">Finding drivers...</span>
            </div>
          ) : riders.length === 0 ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-xs text-gray-400">{rider.plateNumber || 'No Plate'}</span>
                        <span className="w-1 h-1 bg-gray-600 rounded-full"></span>
                        <span className="text-xs text-green-400 font-medium">Online</span>
                    </div>
                  </div>
                </div>
                {selectedRider === rider.id && <CheckCircle className="w-5 h-5 text-blue-500" />}
              </div>
            ))
          )}
        </div>
