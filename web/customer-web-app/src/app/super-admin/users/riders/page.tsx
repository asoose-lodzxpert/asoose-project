...
export default function RidersPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        AppAlert.error('Error', 'Failed to delete rider. Check server connection.');
      }
    }
  };
  ...
  const handleToggleStatus = async (rider: Rider) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const res = await fetch(`${BASE_URL}/super-admin/riders/${rider.id}/status`, { 
            method: 'PATCH',
            headers,
  ...
  const columns = useMemo(() => [
    columnHelper.accessor("name", {
      ...
      cell: info => (
        <Link href={`/super-admin/users/riders/${info.row.original.id}`} className="flex items-center gap-3 group">
           ...
           <div>
              <div className="font-bold text-white text-sm group-hover:text-yellow-500 transition-colors">{info.getValue()}</div>
              <div className="text-[10px] text-gray-500 font-mono uppercase tracking-tighter">{info.row.original.plateNumber}</div>
           </div>
        </Link>
      ),
    }),
    ...
    columnHelper.accessor("walletBalance", {
      header: "Wallet",
      cell: info => <span className="text-white text-sm font-bold"><Currency amount={info.getValue()} /></span>
    }),
    {
      id: 'actions',
      header: 'Manage',
    ...
    <div className="min-h-screen bg-[#0F172A] p-4 md:p-6 pb-20">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold text-white uppercase tracking-tight">Fleet Management</h1>
            <p className="text-sm text-gray-400">Manage riders, payouts, and verifications</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard title="Total Riders" value={stats.total} icon={Users} color="white" isActive={statusFilter === 'ALL'} onClick={() => setStatusFilter('ALL')} />
          <StatCard title="Pending" value={stats.pending} icon={ShieldAlert} color="yellow" isActive={statusFilter === 'PENDING'} onClick={() => setStatusFilter('PENDING')} />
          <StatCard title="Online" value={stats.online} icon={Wifi} color="green" isActive={statusFilter === 'ONLINE'} onClick={() => setStatusFilter('ONLINE')} />
          <StatCard title="Suspended" value={stats.suspended} icon={Ban} color="red" isActive={statusFilter === 'SUSPENDED'} onClick={() => setStatusFilter('SUSPENDED')} />
        </div>
        ...
        <div className="bg-[#1E293B] border border-gray-800 rounded-xl overflow-hidden min-h-[400px]">
           {riders.length === 0 ? (
             <div className="h-[400px] flex flex-col items-center justify-center text-gray-500">
               ...
               <p className="text-sm font-bold uppercase tracking-widest">No riders matched your criteria</p>
             </div>
           ) : (
             <DataTable 
               data={riders} 
                    ...
                    <div className="flex justify-between items-center mb-4">
                       <div className="flex items-center gap-3">
                          ...
                          <div>
                             ...
                             <p className="text-[10px] text-gray-500 font-mono uppercase">{rider.plateNumber}</p>
                          </div>
                       </div>
                       <span className={`text-[9px] font-bold px-2 py-0.5 rounded border ${rider.status === 'ONLINE' ? 'text-green-500 border-green-500/20' : 'text-red-500 border-red-500/20'}`}>
                          {rider.status}
                       </span>
                    </div>
                    <div className="flex justify-between items-center">
                       <Currency amount={rider.walletBalance} className="text-sm font-bold text-white" />
                       <Link href={`/super-admin/users/riders/${rider.id}`} className="text-xs font-bold text-yellow-500">View Dossier</Link>
                    </div>
                 </div>
               )} 
             />
...
const StatCard = ({ title, value, icon: Icon, color, isActive, onClick }: any) => {
  ...
  return (
    <button onClick={onClick} className={`p-5 rounded-xl border text-left transition-all ${isActive ? 'bg-slate-800 border-yellow-500 shadow-lg shadow-yellow-500/5' : 'bg-[#1E293B] border-gray-800 hover:border-gray-700'}`}>
      <div className="flex justify-between mb-3">
        <p className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">{title}</p>
        <div className={`p-1.5 rounded-lg ${styles[color]}`}><Icon className="w-4 h-4" /></div>
      </div>
