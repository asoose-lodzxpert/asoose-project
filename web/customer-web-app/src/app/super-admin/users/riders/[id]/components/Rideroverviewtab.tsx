    ...
    <div className="p-6 space-y-6 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-[#0F172A] p-5 rounded-xl border border-gray-800 flex items-center justify-between shadow-sm">
          <div>
             <p className="text-gray-500 text-[10px] uppercase font-bold tracking-wider">Total Trips</p>
        ...
        <div className="bg-[#0F172A] p-5 rounded-xl border border-gray-800 flex items-center justify-between shadow-sm">
          <div>
             <p className="text-gray-500 text-[10px] uppercase font-bold tracking-wider">Completion Rate</p>
             <span className="text-2xl font-bold text-white mt-1 block">{perf.completionRate || 0}%</span>
          </div>
        ...
        <div className="bg-[#0F172A] p-5 rounded-xl border border-gray-800 flex items-center justify-between shadow-sm">
          <div>
             <p className="text-gray-500 text-[10px] uppercase font-bold tracking-wider">Cancellation Rate</p>
             <span className="text-2xl font-bold text-white mt-1 block">{perf.cancellationRate || 0}%</span>
          </div>
        ...
        <div className="bg-[#0F172A] border border-gray-800 rounded-xl overflow-hidden h-[380px] relative flex flex-col shadow-lg">
          <div className="p-3 border-b border-gray-800 flex justify-between items-center bg-[#1E293B]">
            <h3 className="font-bold text-white flex items-center gap-2 text-xs uppercase tracking-tight">
              ...
              {isRefreshing && <RefreshCw className="w-3.5 h-3.5 animate-spin text-blue-400" />}
            </h3>
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${lastSeen ? 'bg-green-500 animate-pulse' : 'bg-gray-600'}`} />
              <span className="text-[10px] text-gray-400 font-bold uppercase">
                {lastSeen ? `Last Seen: ${new Date(lastSeen).toLocaleTimeString()}` : 'Connection Lost'}
              </span>
            </div>
          </div>
