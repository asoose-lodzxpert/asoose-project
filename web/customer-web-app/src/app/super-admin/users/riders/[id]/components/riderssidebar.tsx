...
import { 
  Mail, Phone, Calendar, Star, ShieldCheck, Ban, CheckCircle, 
  Edit2, Save, X, Loader2, Wallet 
} from 'lucide-react';
...
export const RiderSidebar: React.FC<RiderSidebarProps> = ({ rider, onToggleStatus, onUpdate }) => {
  ...
  const handleManageWallet = async () => {
    const { value: formValues } = await Swal.fire({
      ...
      preConfirm: () => {
        const type = (document.querySelector('input[name="w_type"]:checked') as HTMLInputElement).value;
        const amount = (document.getElementById('w_amount') as HTMLInputElement).value;
        const reason = (document.getElementById('w_reason') as HTMLInputElement).value;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#1E293B] border border-gray-800 rounded-xl p-6 relative overflow-hidden">
        ...
        {rider.verification === 'VERIFIED' && (
          <div className="absolute top-4 right-4 text-blue-400" title="Verified Rider">
        ...
        <div className="flex flex-col items-center text-center mt-6">
          ...
          <div className="w-24 h-24 rounded-full border-4 border-gray-700 bg-gray-800 flex items-center justify-center mb-4 overflow-hidden relative shadow-lg">
             {rider.image ? (
                ...
                <span className="text-3xl font-bold text-gray-500">{rider.name?.charAt(0)}</span>
             )}
          </div>
          ...
          <p className="text-gray-500 text-xs font-mono mt-1 mb-4 bg-gray-800/50 px-2 py-0.5 rounded">{rider.id}</p>

          {/* Rating Pill */}
          <div className="flex items-center gap-1.5 px-3 py-1 bg-[#0F172A] rounded-full border border-gray-800 mb-6">
             <Star className="w-3.5 h-3.5 fill-yellow-500 text-yellow-500" />
             <span className="text-white text-xs font-bold">{rider.rating || 0}</span>
             <span className="text-gray-500 text-[10px]">({rider.totalRides || 0} rides)</span>
          </div>
          ...
          <div className="w-full space-y-3 text-left bg-[#0F172A] p-4 rounded-lg border border-gray-800">
            ...
            <div className="flex items-center gap-3 text-sm text-gray-300">
              ...
              <span>Joined {formatDate(rider.joinedAt || rider.createdAt)}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#1E293B] border border-gray-800 rounded-xl p-6">
        <div className="flex justify-between items-start mb-2">
           <h3 className="text-gray-500 text-xs font-bold uppercase tracking-wider">Wallet Balance</h3>
           <button 
        ...
        <p className={`text-3xl font-black ${rider.walletBalance < 0 ? 'text-red-500' : 'text-green-500'}`}>
