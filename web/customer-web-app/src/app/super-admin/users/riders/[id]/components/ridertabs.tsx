...
import { createRideColumns,createPayoutColumns,getRideStatusIcon,getRideStatusColor } from './columns';
...
export default function RiderTabs({ 
  rides, payouts, onDeleteRide, onProcessPayout, onRetryPayout, onDeletePayout 
}: RiderTabsProps) {
  ...
  const rideColumns = useMemo(() => createRideColumns({ onDelete: onDeleteRide }), [onDeleteRide]);
  const payoutColumns = useMemo(() => createPayoutColumns({ 
    ...
    <div className="bg-[#1E293B] border border-gray-800 rounded-lg p-4 mb-3">
       <div className="flex justify-between items-start mb-3">
          ...
          <div className="flex items-center gap-1">
             ...
             <span className={`px-2 py-1 rounded text-xs font-bold uppercase border ${getRideStatusColor(ride.status)}`}>{ride.status}</span>
          </div>
       </div>
       <div className="space-y-2 text-sm text-gray-300">
          <div><Calendar className="w-4 h-4 inline mr-2"/>{ride.date}</div>
          <div className="flex justify-between items-center pt-2 border-t border-gray-800">
             <span className="font-bold text-white">{ride.fare}</span>
             <button onClick={() => onDeleteRide(ride.id)} className="text-red-400 text-xs flex items-center gap-1">
  ...
  const renderPayoutMobileCard = (payout: Payout) => (
    <div className="bg-[#1E293B] border border-gray-800 rounded-lg p-4 mb-3">
       <div className="flex justify-between items-start mb-3">
          <span className="text-yellow-500 font-mono font-bold text-sm">{payout.id}</span>
          <span className={`px-2 py-1 rounded text-xs font-bold uppercase border ${getPayoutStatusColor(payout.status)}`}>{payout.status}</span>
       </div>
       <div className="flex justify-between items-center text-sm">
          <span className="font-bold text-white">{payout.amount}</span>
          <button onClick={() => onDeletePayout(payout.id)} className="text-red-400 text-xs">
            Delete
          </button>
       </div>
    </div>
        ...
        {['Ride History', 'Payouts', 'Reviews', 'Vehicle Logs'].map((tab) => (
          <button 
            ...
            className={`px-4 md:px-6 py-3 md:py-4 text-sm font-bold whitespace-nowrap transition-colors ${
              activeTab === tab ? 'text-yellow-500 border-b-2 border-yellow-500 bg-[#0F172A]' : 'text-gray-400 hover:text-white hover:bg-white/5'
            }`}
          >
            {tab}
          </button>
        ...
        {activeTab === 'Ride History' && (
          <div className="h-full flex flex-col">
            <div className="flex justify-between items-center mb-4">
               ...
               <Link href="/super-admin/rides" className="text-xs text-yellow-500 hover:text-yellow-400">View All Rides</Link>
            </div>
