...
const StatCard = ({
  ...
  return (
    ...
    >
      <div
        className={`p-3 rounded-lg border ${colorStyles[color as keyof typeof colorStyles]}`}
      >
        <Icon className="w-5 h-5" />
      </div>
      <div>
        <p
          className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${isActive ? "text-yellow-500" : "text-gray-500"}`}
        >
          {title}
        </p>
...
export default function CustomersPage() {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<ApiResponse>(`/super-admin/customers?${queryParams}`, fetcher, {
  ...
  const handleToggleStatus = async (customer: Customer) => {
    ...
    const result = await Swal.fire({
      ...
      text: isBanning
        ? "This user will be restricted from placing orders."
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          title: "Success",
          text: `User has been ${action.toLowerCase()}d`,
          icon: "success",
          toast: true,
          position: "top-end",
          timer: 2000,
    ...
    if (result.isConfirmed) {
      try {
        if (action === "BAN") {
          ...
          await Promise.all(
            selectedIds.map((id) =>
              fetcher(`/super-admin/customers/${id}`, { method: "DELETE" }),
            ),
          );
        }
        setRowSelection({});
    ...
    () => [
      {
        ...
        header: ({ table }) => (
          <input
            ...
            className="rounded border-gray-700 bg-slate-800 text-yellow-500 focus:ring-yellow-500 w-4 h-4"
          />
        ),
        cell: ({ row }) => (
          <input
      ...
      columnHelper.accessor("name", {
        header: ({ column }) => (
          <button
            className="flex items-center gap-1 hover:text-white uppercase text-[10px] tracking-widest font-bold"
        ...
        cell: (info) => (
          <Link
            href={`/super-admin/users/customers/${info.row.original.id}`}
          ...
          >
            ...
            <div>
              <div className="font-bold text-white group-hover:text-yellow-500 transition-colors text-sm">
                {info.getValue()}
              </div>
      ...
      columnHelper.accessor("status", {
        ...
        cell: (info) => {
          ...
          return (
            <span
              className={`px-2 py-0.5 rounded-full text-[9px] font-bold border uppercase tracking-tighter ${color}`}
            >
              {val}
            </span>
          );
      ...
      }),
      {
        ...
        cell: ({ row }) => (
          <div className="flex gap-2">
            <Link
              href={`/super-admin/users/customers/${row.original.id}`}
              className="p-2 hover:bg-slate-700 rounded-lg text-gray-400 hover:text-white transition-all"
            >
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">
              Monitor and manage customer behavior and account status
            </p>
          </div>
          ...
          <div className="bg-yellow-500 text-black p-3 rounded-xl flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="px-4 py-1.5 bg-black text-white text-[10px] font-bold rounded-lg uppercase"
              >
                Bulk Ban
              </button>
              <button
                onClick={() => setRowSelection({})}
                className="px-4 py-1.5 bg-white/20 text-black text-[10px] font-bold rounded-lg uppercase"
              >
                Clear
              </button>
            </div>
            ...
            renderMobileCard={(customer) => (
              <div className="bg-[#0F172A] p-5 rounded-2xl border border-gray-800 mb-3 space-y-4">
                <div className="flex justify-between items-start">
                  ...
                  <span
                    className={`px-2 py-0.5 text-[9px] font-bold rounded border uppercase ${customer.status === "ACTIVE" ? "text-green-500 border-green-500/20" : "text-red-500 border-red-500/20"}`}
                  >
                    {customer.status}
                  </span>
                </div>
