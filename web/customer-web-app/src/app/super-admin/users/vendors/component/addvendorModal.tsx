...
import { X, Loader2, Store, Mail, Tag, Phone, MapPin, Info, Send, User } from 'lucide-react';
...
export default function AddVendorModal({ isOpen, onClose, onVendorAdded }: AddVendorModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onVendorAdded(); 
      onClose();       
      setFormData({ ownerName: '', storeName: '', email: '', category: 'General Goods', phone: '', address: '' });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-[#1E293B] border border-gray-800 w-full max-w-lg rounded-xl md:rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 duration-200">
        ...
        <div className="overflow-y-auto p-4 md:p-6 space-y-6">
          <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-3 flex gap-3 items-start">
            ...
            <div className="text-xs md:text-sm text-blue-200">
              ...
              The vendor will receive an email with their temporary login credentials immediately after creation.
            </div>
          </div>
          ...
          <form id="add-vendor-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-400 uppercase">Store Name <span className="text-red-500">*</span></label>
            ...
            {/* Owner Name */}
            <div className="space-y-1.5">
              <label className="text-xs font-bold text-gray-400 uppercase">Owner Name <span className="text-red-500">*</span></label>
              <div className="relative">
        ...
        <div className="p-4 md:p-6 border-t border-gray-800 bg-[#1E293B] shrink-0 flex gap-3">
          <button type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-600 rounded-lg text-gray-300 font-medium hover:bg-gray-800 transition-colors text-sm">Cancel</button>
          <button type="submit" form="add-vendor-form" disabled={isLoading} className="flex-1 py-2.5 bg-yellow-500 hover:bg-yellow-400 text-black font-bold rounded-lg transition-colors text-sm flex justify-center items-center gap-2 disabled:opacity-50">
            {isLoading ? <><Loader2 className="w-4 h-4 animate-spin" /> Sending Invite...</> : <><Send className="w-4 h-4" /> Send Invite</>}
          </button>
        </div>
      </div>
    </div>
