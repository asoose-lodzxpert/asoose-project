...
export default function VendorManagementPage() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      rejected: allVendors.filter(v => v.status === 'REJECTED' || v.status === 'DISABLED').length
    };
  }, [statsResponse]);
        ...
        const requests = selectedIds.map(id => {
          ...
          if (action === 'approve') {
            return fetch(url, {
              method: 'PATCH',
              headers,
              body: JSON.stringify({ status: 'ACTIVE', verification: 'VERIFIED' })
            });
          } else {
            return fetch(url, { 
              method: 'DELETE',
  ...
  const handleExport = async () => {
    try {
      const headers = await getAuthHeader();
      const res = await fetch(`${API_URL}/super-admin/vendors?limit=10000`, { headers });
      const allData: VendorsApiResponse = await res.json();
      
      const csv = [
        ['Name', 'Email', 'Category', 'Status', 'Verification', 'Rating', 'Orders'].join(','),
        ...allData.data.map(v => [
          v.name, 
          v.email, 
          v.category, 
  ...
  const columns = useMemo<ColumnDef<Vendor, any>[]>(() => [
        ...
        <div className="flex flex-col">
          <span className="font-bold text-white truncate flex items-center gap-2">
            ...
            {info.row.original.status === 'PENDING' && (
              <span className="text-[10px] bg-blue-500 text-white px-1 rounded animate-pulse">NEW</span>
            )}
          </span>
          <span className="text-xs text-gray-500 font-mono">ID: {info.row.original.id.substring(0,8)}...</span>
        </div>
      ),
    }),
    columnHelper.accessor('email', {
      ...
      cell: info => (
        <div className="flex items-center gap-2 group">
          <span className="text-gray-400 truncate max-w-[150px]">{info.getValue()}</span>
          <button 
    ...
    columnHelper.accessor('status', {
      ...
      cell: info => {
        ...
        return (
          <span className={`px-2 py-1 rounded text-xs font-bold uppercase border ${colors[status.toUpperCase() as keyof typeof colors] || colors.PENDING}`}>
            {status}
          </span>
        );
      },
  ...
  return (
    <div className="min-h-screen bg-[#0F172A] p-4 md:p-6 overflow-x-hidden relative pb-20">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-6">
          <div className="flex flex-col md:flex-row justify-between items-start gap-4">
            <div>
              ...
              <p className="text-sm text-gray-400 mt-1">Overview of all service providers</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 cursor-pointer">
            <StatCard 
              ...
              active={statusFilter === 'All Status' && verificationFilter === 'All'}
            />
            <StatCard 
        ...
        <div className={`bg-[#1E293B] p-4 rounded-xl border border-gray-800 flex flex-col md:flex-row gap-4 items-end transition-all duration-300 ${showFilters ? 'flex' : 'hidden'} md:flex`}>
          ...
          {(searchQuery || statusFilter !== 'All Status' || verificationFilter !== 'All') && (
            <button 
