    ...
    <div className="sticky top-0 z-20 bg-[#0F172A]/95 backdrop-blur-md border-b border-gray-800 py-4 -mx-4 px-4 md:-mx-6 md:px-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      <div>
        ...
        <div className="flex items-center gap-3">
          ...
          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase border ${status === 'ACTIVE' ? 'bg-green-500/20 text-green-500 border-green-500/20' : 'bg-yellow-500/20 text-yellow-500 border-yellow-500/20'}`}>
            {status}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        {isEditing ? (
          ...
          <button onClick={onEdit} className="flex items-center gap-2 px-4 py-2 bg-yellow-500 text-black font-bold rounded-lg hover:bg-yellow-400 text-sm">
