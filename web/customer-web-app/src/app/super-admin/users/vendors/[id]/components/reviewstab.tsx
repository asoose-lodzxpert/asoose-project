  ...
  const handleDelete = (id: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Swal.fire({ title: 'Deleted', icon: 'success', background: '#1E293B', color: '#fff', confirmButtonColor: '#eab308' });
      }
    });
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4">
        {reviews.map((review) => (
          <div key={review.id} className="bg-[#0F172A] border border-gray-800 p-4 rounded-xl hover:border-gray-700 transition-all">
            ...
            <div className="flex gap-3 border-t border-gray-800 pt-3">
               <button onClick={() => handleDelete(review.id)} className="text-xs text-red-400 hover:text-red-300 flex items-center gap-1 font-bold">
