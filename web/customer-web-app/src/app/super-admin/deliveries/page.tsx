...
import { 
  ...
  FileSpreadsheet, AlertTriangle, RefreshCw
} from 'lucide-react';
...
const DeliveryMap = ({ deliveries }: { deliveries: Delivery[] }) => (
  <div className="bg-[#1E293B] border border-gray-800 rounded-xl h-[600px] relative overflow-hidden group">
    <div className="absolute inset-0 bg-[#0F172A]" style={{ backgroundImage: 'radial-gradient(#334155 1px, transparent 1px)', backgroundSize: '20px 20px' }}>
       {/* Mock Pins */}
       {deliveries.slice(0, 5).map((d, i) => (
          <div key={d.id} className="absolute flex flex-col items-center gap-1 cursor-pointer hover:z-10 group/pin" 
    ...
    <div className="absolute bottom-4 left-4 bg-gray-900/90 p-4 rounded-lg border border-gray-800 backdrop-blur text-xs">
       ...
       <div className="flex gap-4">
          <span className="flex items-center gap-1 text-blue-400"><div className="w-2 h-2 rounded-full bg-blue-500" /> Moving</span>
          <span className="flex items-center gap-1 text-yellow-400"><div className="w-2 h-2 rounded-full bg-yellow-500" /> Idle</span>
          <span className="flex items-center gap-1 text-green-400"><div className="w-2 h-2 rounded-full bg-green-500" /> Delivered</span>
       </div>
    </div>
  </div>
);
...
export default function DeliveriesPage() {
  ...
  const { data: apiResponse, error, isLoading, mutate } = useSWR<ApiResponse>(
  ...
  const getStatusColor = (status: string) => {
    ...
    if (s === 'IN TRANSIT' || s === 'PICKED_UP') return 'bg-blue-500/10 text-blue-400 border-blue-500/20';
    if (s === 'PENDING PICKUP' || s === 'ASSIGNED') return 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20';
  ...
  const columns = useMemo<ColumnDef<Delivery>[]>(() => [
    {
      ...
      cell: ({ row }) => (
        <div>
           <Link href={`/super-admin/deliveries/${row.original.id}`} className="text-yellow-500 font-mono font-bold text-xs hover:underline block">
    ...
    },
    {
      ...
      header: 'Status',
      cell: ({ row }) => (
        <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border whitespace-nowrap ${getStatusColor(row.original.status)}`}>
          {row.original.status}
        </span>
      ),
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2 text-white">
          {row.original.type?.includes('Document') ? <FileText className="w-4 h-4 text-blue-400" /> : <Package className="w-4 h-4 text-orange-400" />}
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex flex-col gap-1 text-xs max-w-[180px]">
          <span className="text-gray-400 truncate" title={row.original.pickup}>From: <span className="text-white">{row.original.pickup}</span></span>
          <span className="text-gray-400 truncate" title={row.original.dropoff}>To: <span className="text-white">{row.original.dropoff}</span></span>
        </div>
      ),
    },
    {
      ...
      header: 'Driver',
      cell: ({ row }) => (
        <span className={`text-sm ${row.original.driver === '-' ? 'text-gray-500 italic' : 'text-gray-300'}`}>
          {row.original.driver}
        </span>
      ),
    },
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
            {(row.original.status === 'Pending Pickup' || row.original.driver === '-') ? (
               ...
               <Link href={`/super-admin/deliveries/${row.original.id}`} className="p-1.5 hover:bg-blue-500/10 rounded text-gray-400 hover:text-blue-500 transition-colors">
    ...
    <div className="min-h-screen bg-[#0F172A] p-4 md:p-6 pb-20">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">Logistics & Deliveries</h1>
            <p className="text-gray-400 text-sm mt-1">Manage fleet movements and assignments</p>
          </div>
        ...
        {/* Filter Bar */}
        <div className={`bg-[#1E293B] p-4 rounded-xl border border-gray-800 grid grid-cols-1 md:grid-cols-4 gap-4 ${filterOpen ? 'block' : 'hidden md:grid'}`}>
           ...
           <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="bg-[#0F172A] text-gray-300 text-sm border border-gray-700 rounded-lg p-2 outline-none">
              <option value="All">All Status</option><option value="In Transit">In Transit</option><option value="Pending Pickup">Pending Pickup</option><option value="Delivered">Delivered</option>
           </select>

           <div className="flex gap-2 md:col-span-2">
              <input type="date" value={dateRange.from} onChange={e => setDateRange(p => ({...p, from: e.target.value}))} className="w-full bg-[#0F172A] text-gray-300 text-sm border border-gray-700 rounded-lg px-2" />
              <input type="date" value={dateRange.to} onChange={e => setDateRange(p => ({...p, to: e.target.value}))} className="w-full bg-[#0F172A] text-gray-300 text-sm border border-gray-700 rounded-lg px-2" />
           </div>
        </div>
        ...
        {Object.keys(rowSelection).length > 0 && viewMode === 'list' && (
           <div className="bg-blue-500/10 border border-blue-500/20 p-3 rounded-xl flex justify-between items-center animate-in fade-in slide-in-from-top-2">
              <span className="text-blue-400 text-sm font-bold ml-2">{Object.keys(rowSelection).length} selected</span>
              <div className="flex gap-2">
                 <button onClick={handleBulkManifest} className="flex items-center gap-2 px-3 py-1.5 bg-blue-500 text-white text-xs font-bold rounded hover:bg-blue-600">
        ...
        {error && (
          <div className="p-4 bg-red-500/10 border border-red-500/20 text-red-400 rounded-lg text-center">
             ...
             <button onClick={() => mutate()} className="ml-2 underline hover:text-red-300">Retry</button>
          </div>
        )}
