...
export default function TransactionDetailPage() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/super-admin/transactions/${txn.id}/receipt`,
        {
          method: "GET",
          headers: {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_URL}/super-admin/transactions/${transactionId}/verify`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token || ""}`,
  ...
  if (error || !txn) {
    return (
      <div
        className={`min-h-screen bg-[#0F172A] flex items-center justify-center ${inter.className}`}
      >
  ...
  const canVerify =
    ["PENDING", "FAILED", "PROCESSING"].includes(txn.status) && !!txn.reference;
