...
import { FileText, Eye, CheckCircle, XCircle, Download, Upload, AlertTriangle, Clock } from 'lucide-react';
...
export default function DocumentsTab({ documents, onVerify, onReject, showUploadButton = false }: DocumentsTabProps) {
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
      ...
      {(!documents || documents.length === 0) ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {documents.map((doc) => (
            <div key={doc.id} className="bg-[#0F172A] border border-gray-800 rounded-xl p-4 flex flex-col gap-4 group hover:border-gray-700 transition-all shadow-lg shadow-black/20">
              ...
              <div className="flex justify-between items-start">
                ...
                <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase border ${getStatusStyle(doc.status)}`}>
                  {doc.status}
                </span>
              </div>
              ...
              {doc.status === 'REJECTED' && doc.rejectionReason && (
                 <div className="bg-red-500/5 border border-red-500/10 p-2.5 rounded-lg text-xs text-red-200 flex items-start gap-2">
                    ...
                    <span><span className="font-bold text-red-400">Reason:</span> {doc.rejectionReason}</span>
                 </div>
              )}

              {/* Action Buttons */}
              <div className="flex gap-2 mt-auto pt-2 border-t border-gray-800/50">
                 <button className="p-2 text-gray-500 hover:text-white hover:bg-gray-800 rounded-lg transition-colors" title="Download">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-sm animate-in fade-in duration-200">
           ...
           <div className="max-w-3xl w-full max-h-[85vh] flex flex-col gap-4">
              <div className="flex items-center justify-between text-white">
                 <h3 className="text-xl font-bold capitalize">{selectedDoc.type.replace(/_/g, ' ')}</h3>
                 <span className={`px-3 py-1 rounded-full text-xs font-bold border uppercase ${getStatusStyle(selectedDoc.status)}`}>
                    {selectedDoc.status}
                 </span>
              </div>
              ...
              <div className="relative w-full h-[65vh] bg-black rounded-xl border border-gray-800 overflow-hidden flex items-center justify-center">
                 {/* eslint-disable-next-line @next/next/no-img-element */}
                 <img src={selectedDoc.url} alt="Doc" className="max-w-full max-h-full object-contain" />
              </div>

              {selectedDoc.status === 'PENDING' && (
                <div className="flex justify-center gap-4">
                   <button 
                     onClick={() => { handleRejectClick(selectedDoc); setSelectedDoc(null); }}
                     className="px-8 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold shadow-lg shadow-red-500/20"
                   >
                     Reject Document
                   </button>
                   <button 
                     onClick={() => { onVerify(selectedDoc.id); setSelectedDoc(null); }}
