...
const OrderStepper = ({ status }: { status: string }) => {
    ...
    if (['CANCELLED', 'REJECTED'].includes(status)) {
        return (
            <div className="w-full bg-red-500/10 border border-red-500/20 p-4 rounded-lg flex items-center justify-center gap-2 text-red-500 font-bold mb-6">
    ...
    return (
        <div className="w-full bg-[#1E293B] p-6 rounded-xl border border-gray-800 mb-6 overflow-x-auto print:hidden shadow-sm">
...
export default function OrderDetailsPage() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 pb-20 print:p-0 print:bg-white">
      ...
      {/* 1. Sticky Header */}
      <div className="flex flex-col md:flex-row justify-between items-start gap-4 print:hidden sticky top-0 z-20 bg-[#0F172A]/90 backdrop-blur-md py-4 border-b border-gray-800">
        <div>
           <Link href="/super-admin/orders" className="text-gray-400 hover:text-white flex items-center gap-1 text-xs font-bold uppercase transition-colors mb-1">
        ...
        <div className="flex gap-2">
           ...
           <button onClick={() => window.print()} className="px-5 py-2.5 bg-white text-black rounded-lg text-xs font-bold flex items-center gap-2 hover:bg-gray-200 transition-all shadow-sm">
      ...
      {/* 2. Dispute Alert */}
      {order.dispute && (
        <div className="bg-red-500 border border-red-400 p-4 rounded-xl flex flex-col md:flex-row items-center gap-4 text-white shadow-md">
            <AlertTriangle className="w-8 h-8 flex-shrink-0" />
            <div className="flex-1 text-center md:text-left">
                <h3 className="font-bold uppercase text-sm">Active Dispute Reported</h3>
                <p className="text-sm opacity-90">Reason: {order.dispute.reason}</p>
            </div>
            <Link href={`/super-admin/disputes/${order.dispute.id}`} className="px-6 py-2 bg-white text-red-600 text-xs font-bold rounded-lg hover:bg-gray-100 transition-all uppercase">
              View Dispute
            </Link>
        </div>
      )}
                ...
                {order.items.map((item, i) => (
                  <div key={i} className="py-4 flex items-center justify-between">
                      <div className="flex items-center gap-4">
                          <div className="w-12 h-12 bg-slate-900 rounded-lg flex items-center justify-center border border-slate-800 overflow-hidden">
                             {item.image ? (
                          ...
                          <div>
                             ...
                             <p className="text-gray-500 text-[10px] font-medium uppercase">
                                 {item.options || 'Standard'} • x{item.quantity}
                             </p>
                          </div>
                      </div>
                      <div className="text-right">
                          <p className="text-white text-sm font-bold"><Currency amount={item.price * item.quantity} /></p>
                          <p className="text-gray-500 text-[10px] font-bold"><Currency amount={item.price} /> Each</p>
                      </div>
                  </div>
                ))}
              </div>
              <div className="bg-slate-900/50 p-5 space-y-2 border-t border-slate-800">
              ...
              <div className="space-y-6">
                 {order.logs.map((log, i) => (
                    <div key={i} className="flex gap-4 relative">
                        ...
                        <div className="flex-1 bg-slate-900/40 p-4 rounded-xl border border-slate-800/50">
                          ...
                          <p className="text-[10px] text-gray-400">Performed by <span className="text-blue-400 font-bold">{log.user}</span> • {new Date(log.date).toLocaleDateString()}</p>
                          {log.details && (
                             <div className="mt-2 text-[10px] text-gray-400 bg-black/20 p-2 rounded border border-slate-800">
                                {log.details}
                             </div>
                          )}
                        </div>
        ...
        <div className="lg:col-span-1 space-y-6 print:hidden">
           ...
           <div className="bg-[#1E293B] border border-slate-800 rounded-xl p-5 shadow-sm">
              ...
              <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800 mb-4">
                  <p className="text-white font-bold text-lg mb-1">{order.customer.name}</p>
              ...
              <div className="space-y-3">
                  ...
                  <div className="p-3 bg-slate-900 border border-slate-800 rounded-lg">
                      <div className="flex gap-3">
                          ...
                          <span className="text-gray-300 text-xs leading-relaxed font-medium">{order.customer.address}</span>
                      </div>
                  </div>
              </div>
           </div>
           ...
           <div className="bg-[#1E293B] border border-slate-800 rounded-xl p-5 shadow-sm">
              ...
              <div className="space-y-4">
                  ...
                  <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800">
                      ...
                      <p className="text-white font-bold text-sm mb-1">{order.vendor.name}</p>
                  ...
                  {order.rider ? (
                      <div className="p-4 bg-slate-900/50 rounded-lg border border-slate-800">
                          ...
                          <p className="text-white font-bold text-sm mb-1">{order.rider.name}</p>
                          <p className="text-gray-500 text-[10px] font-bold">{order.rider.vehicle} • {order.rider.phone}</p>
                          <div className="flex gap-2 mt-3">
