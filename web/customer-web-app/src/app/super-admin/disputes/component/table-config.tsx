...
export const disputeColumns = [
        ...
        <div
          className={`w-1 h-8 rounded-full ${
            info.row.original.breachedSLA
              ? "bg-red-500"
              : info.row.original.priority === "URGENT"
                ? "bg-orange-500"
                : "bg-gray-700"
          }`}
        />
  ...
  columnHelper.accessor("status", {
    ...
    cell: (info) => (
      <div className="flex items-center gap-1">
        <span
          className={`px-2 py-1 rounded text-[10px] font-bold uppercase border flex items-center gap-1 ${getStatusColor(info.getValue())}`}
        >
  ...
  columnHelper.accessor("priority", {
    ...
    cell: (info) => (
      <span
        className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${getPriorityColor(info.getValue())}`}
      >
        {info.getValue()}
      </span>
    ),
  ...
  columnHelper.accessor("parties", {
    ...
    cell: (info) => (
      <span
        className="text-gray-400 text-xs truncate max-w-[180px] block"
        title={info.getValue()}
      >
  ...
  columnHelper.accessor("hoursOpen", {
    ...
    cell: (info) => (
      <div className="flex flex-col">
        <span
          className={`text-xs font-mono font-bold ${info.getValue() > 48 ? "text-red-400" : "text-gray-300"}`}
        >
          {info.getValue()}h
        </span>
...
export const renderMobileDisputeCard = (dispute: Dispute) => (
  <div
    className={`bg-[#1E293B] border-l-4 ${dispute.breachedSLA ? "border-l-red-500" : dispute.priority === "URGENT" ? "border-l-orange-500" : "border-l-gray-700"} border-y border-r border-gray-800 rounded-r-lg p-4 mb-3 shadow-lg`}
  >
    <div className="flex justify-between items-start mb-3">
      ...
      <span
        className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${getPriorityColor(dispute.priority)}`}
      >
        {dispute.priority}
      </span>
    </div>
    <div className="grid grid-cols-2 gap-2 text-xs text-gray-400 mb-3">
      <div>
        Value: <span className="text-white">{dispute.relatedAmount}</span>
      </div>
      <div>
        Open:{" "}
        <span
          className={dispute.hoursOpen > 48 ? "text-red-400" : "text-white"}
        >
          {dispute.hoursOpen}h
        </span>
      </div>
