...
export default function DisputeChat({ messages, canAddMessage, onSendMessage }: Props) {
  ...
  return (
    <div className="bg-[#1E293B] border border-gray-800 rounded-2xl p-6 shadow-xl">
      ...
      <div className="space-y-6 mb-6 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
        {messages.length === 0 ? (
          <div className="text-center py-10 text-gray-500 italic">No messages yet.</div>
        ) : (
          messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                <div className={`flex items-center gap-2 mb-1 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                  ...
                  <span className="text-[10px] text-gray-500 uppercase bg-gray-800 px-1 rounded">{msg.sender.role}</span>
                ...
                <div className={`max-w-[85%] rounded-2xl px-4 py-3 text-sm whitespace-pre-wrap shadow-sm ${
                  msg.isInternal ? 'bg-yellow-900/20 border border-yellow-700/50 text-yellow-100' 
      ...
      {canAddMessage && (
        <div className="relative bg-[#0F172A] rounded-xl border border-gray-700 p-2">
          ...
          <div className="flex justify-between items-center px-2 pb-1">
            ...
            >
              {isInternal ? <AlertCircle className="w-3 h-3" /> : <AlertTriangle className="w-3 h-3" />}
