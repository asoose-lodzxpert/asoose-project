    ...
    <div className="bg-[#1E293B] border border-gray-800 rounded-2xl p-6 shadow-xl">
      <div className="flex items-start justify-between mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <span
            className={`px-3 py-1 rounded text-xs font-bold uppercase border flex items-center gap-1 ${getStatusColor(dispute.status)}`}
          >
            {dispute.status}
          </span>
        </div>
      ...
      <div className="space-y-5 text-sm">
        {/* Reason & Desc */}
        <div>
          <span className="text-gray-400 block mb-1 text-xs uppercase tracking-wider">
            Issue
          </span>
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="overflow-hidden">
                <span className="text-white font-medium block truncate">
                  {dispute.openedByUser.name}
                </span>
                <div className="flex items-center gap-1 text-gray-500 text-xs">
                  <span className="truncate">{dispute.openedByUser.email}</span>
                  <button
                    onClick={() =>
                      copyToClipboard(dispute.openedByUser.email, "Email")
                    }
                  >
