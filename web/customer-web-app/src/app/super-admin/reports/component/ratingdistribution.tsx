...
export default function RatingsDistribution({ ratings, avgRating }: RatingsProps) {
  return (
    <div className="bg-[#1E293B] p-6 rounded-xl border border-gray-800">
      ...
      <div className="space-y-4">
        {ratings.map((item) => (
          <div key={item.star} className="flex items-center gap-3">
            <span className="text-sm font-bold w-4 text-white">{item.star}</span>
      ...
      <div className="mt-8 text-center">
        ...
        <p className="text-xs text-gray-400">Average rating based on 12,450 reviews</p>
      </div>
    </div>
  );
}