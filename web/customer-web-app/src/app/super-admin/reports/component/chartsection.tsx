  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#0F172A] border border-gray-700 rounded-lg p-3 shadow-xl z-50">
        <p className="text-gray-300 text-xs font-bold mb-2 border-b border-gray-800 pb-1">
          {label}
        </p>
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            ...
            <p className="text-xs text-gray-400 flex items-center gap-1 mt-1">
              <Info className="w-3 h-3" /> Grouped by{" "}
              <span className="text-yellow-500 font-bold">{granularity}</span>
            </p>
          </div>
        </div>
        ...
        <div className="h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart
              data={volumeData}
              onClick={(data) =>
                console.log("Drill down to date:", data?.activeLabel)
              }
            >
              <CartesianGrid
                strokeDasharray="3 3"
